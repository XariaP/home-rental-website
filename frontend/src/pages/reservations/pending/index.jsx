...
function Pending({reservation, propertyDetails, userType}) {
    ...
    function handleApprove() {
        ...
        if (confirmResult) {
            fetch(`http://localhost:8000/reservations/${reservation.id}/approve/`,
            {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
    ...
    function handleDeny() {
        const confirmResult = window.confirm("Are you sure you want to continue?\nThis will deny the reservation.");
        if (confirmResult) {
            fetch(`http://localhost:8000/reservations/${reservation.id}/deny/`,
            {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
    ...
    function ButtonManagement() {
        ...
        if (userType === "host") {
            ...
            <div>
                ...
                <button className="btn btn-success my-2" onClick={handleApprove}>Approve</button>
                <button className="btn btn-danger my-2" onClick={handleDeny}>Deny</button>
            </div>
            </>
        }
        else {
    ...
    return <>
        <div className="row">
            ...
            <div className="summaryinfo col-md">
                <h3 className="fw-light padded">Request to book {propertyDetails[reservation.id] && propertyDetails[reservation.id].name}</h3>
                <div className="icons">
                    <p className="fw-medium padded nonmargin">Status:</p>
                    <div className="icons padded">
        
                        <p className="fw-light nonmargin">Pending</p>
