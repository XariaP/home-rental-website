...
function Approved({reservation, propertyDetails, userType}) {

    const { token } = useContext(UserContext);
    const navigate = useNavigate();

    function handleProfile() {
        // window.location.href = `/accounts/profile/${reservation.renter}/view/renter`;
    ...
    function handleCancel() {
        const confirmResult = window.confirm("Are you sure you want to continue?\nThis will send a request to cancel the reservation.");
        if (confirmResult) {
            fetch(`http://localhost:8000/reservations/${reservation.id}/cancel/`,
            {
                method: 'GET',
                headers: {
    ...
    function handleTerminate() {
        const confirmResult = window.confirm("Are you sure you want to continue?\nThis will terminate the reservation.");
        if (confirmResult) {
            fetch(`http://localhost:8000/reservations/${reservation.id}/terminate/`,
            {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
    ...
    function ButtonManagement() {
        if (userType === "host") {
            ...
            <div>
                ...
                <button className="btn btn-danger my-2" onClick={handleTerminate}>Terminate</button>
            </div>
            </>
        }
        else {
            return <>
                <div>
                    <button className="btn btn-danger my-2" onClick={handleCancel}>Request Cancellation</button>
                </div>
            </>
        }
    }
    ...
    return <>
        <div className="row">
            ...
            <div className="summaryinfo col-md">
                <h3 className="fw-light padded">Reserved {propertyDetails[reservation.id] && propertyDetails[reservation.id].name}</h3>
                <div className="icons">
                    <p className="fw-medium padded nonmargin">Status:</p>
                    <div className="icons padded">
        
                        <p className="fw-light nonmargin">Approved!</p>
